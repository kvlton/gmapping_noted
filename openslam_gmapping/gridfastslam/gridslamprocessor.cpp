...
(double srr, double srt, double str, double stt){
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles)
  {
    ...
    //invoke the callback
    /*回调函数 实际上什么都没做 表示里程计更新完成*/
    onOdometryUpdate();
    

    // accumulate the robot translation and rotation
    /*对机器人的线性位移和角度位移进行累加，直到进行了激光雷达的更新再归零*/
    OrientedPoint move=relPose-m_odoPose;
    move.theta=atan2(sin(move.theta), cos(move.theta));
    m_linearDistance+=sqrt(move*move);   // 线性位移累加
    m_angularDistance+=fabs(move.theta); // 角度位移累加
    
    // if the robot jumps throw a warning
    /*机器人走了很远都没有进行激光雷达的更新 则进行报警*/
    ...
    /*只有当机器人走过一定的距离  或者 旋转过一定的角度  或者过一段指定的时间 才处理激光数据*/
    if (! m_count 
    ...
    m_readingCount++; // 所有的激光数据帧数
    return processed; // 是否对激光数据进行处理
  }
